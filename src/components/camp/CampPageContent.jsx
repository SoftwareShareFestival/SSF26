...
export default function CampPageContent({ id }) {
	...
	const safeIndex =
		listLength > 0 ? ((currentIndex % listLength) + listLength) % listLength : 0;
	...
	useEffect(() => {
		function updateTutorialTop() {
			...
			const img =
				itemRef.current.querySelector && itemRef.current.querySelector("img");
						...
						<div className="camp-scrollbar pb-2 px-2">
							<div className="relative overflow-hidden">
										...
										<Link
											...
											className={
												isMobileDevice
													...
													: "min-w-[200px] max-w-[200px] flex-shrink-0 flex flex-col items-start text-left"
											}
										>
								...
								{isMobileDevice && (
									<>
										<div
											className={`edge-shadow-right ${showRightEdgeShadow ? "visible" : ""}`}
										/>
										{/* {showTutorial && currentIndex === 0 && (
											<div
												className="tutorial-arrow"
												aria-hidden
												style={{ top: tutorialTop ? `${tutorialTop}px` : "50%" }}
											>
												<svg
							...
							<div className="flex justify-center items-center gap-1.5 mt-5">
								{filteredCamps.map((_, index) => (
									<button
										key={index}
										onClick={() => setCurrentIndex(index)}
										className={`h-1.5 rounded-full transition-all duration-300 ${
											currentIndex === index ? "bg-[#ff3b8d] w-4" : "bg-[#D1D1D6] w-1.5"
										}`}
									/>
								))}
							</div>
